...
namespace RazorPagesProject.IntegrationTests.Helpers;
...
public static class HttpClientExtensions
{
        ...
        IEnumerable<KeyValuePair<string, string>> formValues)
    {
        ...
        var submit = form.GetSubmission(submitButton) ?? throw new InvalidOperationException("submit");        
        var target = (Uri)submit.Target;
        if (submitButton.HasAttribute("formaction"))
        {
            var formaction = submitButton.GetAttribute("formaction") ?? throw new InvalidOperationException("formaction");
